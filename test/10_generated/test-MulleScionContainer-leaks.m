...
static void test_MulleScionArray_noleak(void)
{
   @autoreleasepool
   {
      @try
      {
         [[MulleScionArray newWithArray:@[@1, @2] lineNumber:1] autorelease];
      }
...
static void test_MulleScionDictionary_noleak(void)
{
   @autoreleasepool
   {
      @try
      {
         [[MulleScionDictionary newWithDictionary:@{@"key": @"value"} lineNumber:1] autorelease];
      }
      @catch( NSException *localException)
      {
         fprintf( stderr, "Threw a %s exception\n", [[localException name] UTF8String]);
